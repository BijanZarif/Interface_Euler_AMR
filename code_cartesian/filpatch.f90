...
!   integer, intent(in) :: fill_indices(4)
    ...
    real(kind=8) :: valp20, valm20, valp02, valm02
    
    ! To check interpolation for aux
    real(kind=8) :: gamma, gammap10, gammam10, gammac, gammap01, gammam01
...
!!$                          * (fill_indices(4) - fill_indices(3) + 3) *naux)
    ...
    real(kind=8) :: gamma_coarse(ihi-ilo + 3, jhi-jlo + 3)
...
!!$                fill_indices(2),fill_indices(3),fill_indices(4),level,nvar,naux)
    ...
    if (.not.set) then
        ...
        ! Write coarde values of gamma to read later on
        do j_coarse = 1, my_coarse
            do i_coarse = 1, mx_coarse
                gamma = auxcrse(iauxc(i_coarse,j_coarse))
                gamma_coarse(i_coarse,j_coarse) = gamma
            enddo
        enddo
        ...
        do i_fine = 1,mx_patch
            ...
            do j_fine  = 1,my_patch
                ...
                if (flaguse(i_fine,j_fine) == 0) then
                    ...
                    ! Edited my @maojrs to interpolate correctly at interfaces with big jumps
                    gammap10 = gamma_coarse(i_coarse + 1 ,j_coarse)
                    gammam10 = gamma_coarse(i_coarse - 1,j_coarse)
                    gammac   = gamma_coarse(i_coarse,j_coarse)
                    gammap01 = gamma_coarse(i_coarse,j_coarse + 1)
                    gammam01 = gamma_coarse(i_coarse,j_coarse - 1)
                    ...
                    do n=1,nvar
                        ...
                        if (isnan(valc)) then                     
                            print*, valp10, valc, valm10
                            stop
                        end if

                        ! If no interface do usual interpolation, else do minmod one sided (@maojrs)
                        if ((gammap10 .eq. gammac) .and. (gammam10 .eq. gammac)) then
                            dupc = valp10 - valc
                            dumc = valc   - valm10
                            ducc = valp10 - valm10
                            ...
                            if (gammap10 .eq. gammac) then
                                if (i_coarse+2 <= mx_coarse) then
                                    valp20 = valcrse(coarse_index(n,i_coarse + 2,j_coarse))
                                else
                                    valp20 = valcrse(coarse_index(n,i_coarse + 1,j_coarse))
                                end if
!                                 dupc = valp20 - valp10
!                                 dumc = valp10 - valc
!                                 ducc = valp20 - valc
...
!                                 fu = max(0.d0, sign(1.d0, dupc * dumc))
                                fu = 1.d0
                                du = 1.0*abs(valp10 - valc)
                                ducc = sign(1.d0,du)
                            else
                                if (i_coarse-2 >= 1) then
                                    valm20 = valcrse(coarse_index(n,i_coarse - 2,j_coarse))
                                else
                                    valm20 = valcrse(coarse_index(n,i_coarse - 1,j_coarse))
                                end if
!                                 dupc = valc - valm10
!                                 dumc = valm10 - valm20
!                                 ducc = valc - valm20
...
!                                 fu = max(0.d0, sign(1.d0, dupc * dumc))
                                fu = 1.d0
                                du = 1.0*abs(valc - valm10)
                                ducc = sign(1.d0,du)
                            end if
                        end if
                        
                        if ((gammap01 .eq. gammac) .and. (gammam01 .eq. gammac)) then
                            dvpc = valp01 - valc
                            dvmc = valc   - valm01
                            dvcc = valp01 - valm01
                            ...
                            if (gammap01 .eq. gammac) then
                                if (j_coarse+2 <= my_coarse) then
                                    valp02 = valcrse(coarse_index(n,i_coarse,j_coarse + 2))
                                else
                                    valp02 = valcrse(coarse_index(n,i_coarse,j_coarse + 1))
                                end if
!                                 dvpc = valp02 - valp01
!                                 dvmc = valp01 - valc
!                                 dvcc = valp02 - valc
...
!                                 fv = max(0.d0, sign(1.d0, dvpc * dvmc))
                                fv = 1.d0
                                dv = 1.0*abs(valp01 - valc)
                                dvcc = sign(1.d0,dv)
                            else
                                if (j_coarse-2 >= 1) then
                                    valm02 = valcrse(coarse_index(n,i_coarse,j_coarse - 2))
                                else
                                    valm02 = valcrse(coarse_index(n,i_coarse,j_coarse - 1))
                                end if
!                                 dvpc = valc - valm01
!                                 dvmc = valm01 - valm02
!                                 dvcc = valc - valm02
